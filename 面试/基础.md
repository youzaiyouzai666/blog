...
### ‌SSE 与 WebSocket 在流式通信中的核心区别 ‌

‌1. 通信模式 ‌
‌SSE‌

- ‌ 单向通信 ‌：仅支持服务器向客户端单向推送数据，客户端无法通过 SSE 主动发送请求 13。
- ‌ 基于 HTTP 协议 ‌：通过 HTTP 长连接实现数据推送，无需额外协议升级 12。
  ‌WebSocket‌
  ‌- 全双工通信 ‌：支持客户端与服务器双向实时交互，双方可同时发送和接收数据 13。
- ‌ 独立协议 ‌：基于 TCP 协议，需通过 HTTP 协议升级握手建立连接，后续传输独立于 HTTP12。
